...
namespace ProBuilder2.Actions
{
	public class DetachDeleteFace : Editor
	{
		...
		// [MenuItem("Tools/" + pb_Constant.PRODUCT_NAME + "/Geometry/New pb_Object with Faces", false, pb_Constant.MENU_GEOMETRY + pb_Constant.MENU_GEOMETRY_FACE+6)]
		// public static void MenuDetachFaces()
		// {
		// 	pb_Object[] pbSelection = pbUtil.GetComponents<pb_Object>(Selection.transforms);

		// 	pbUndo.RecordObjects(pbSelection as Object[], "Detach Faces");
		// 	foreach(pb_Object pb in pbSelection)
		// 	{
		// 		pb_Object[] results = pb.DetachFaces(pb.SelectedFaces);
		// 		pb_Editor.instance.ClearFaceSelection();
		// 		pb_Editor.instance.UpdateSelection();
		// 		foreach(pb_Object p in results)
		// 		{
		// 			p.Refresh();
		// 			p.GenerateUV2(true);
		// 		}
